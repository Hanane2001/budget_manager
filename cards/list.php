...
$result = $conn->query("SELECT * FROM cards WHERE idUser = $userId ORDER BY isMain DESC, created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8">
            <div><h1 class="text-3xl font-bold text-gray-800">Card Management</h1></div>
        ...
        <div id="addForm" class="hidden bg-white rounded-xl shadow p-6 mb-8">
            ...
            <form action="create.php" method="POST" class="space-y-4">
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-gray-700 mb-2">Card Number (Optional)</label>
                        <input type="text" name="cardNumber" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="**** **** **** 1234">
                    </div>
                    <div>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            <?php if ($result->num_rows > 0): ?>
                ...
                <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl shadow p-6 text-white">
                    <div class="flex justify-between items-center mb-4">
                        <div>
                            <h3 class="font-bold text-lg"><?php echo htmlspecialchars($card['cardName']); ?></h3>
                            <p class="text-sm opacity-80"><?php echo htmlspecialchars($card['bankName']); ?></p>
                            <?php if ($card['cardNumber']): ?>
                                <p class="text-sm opacity-80 mt-1"><?php echo htmlspecialchars($card['cardNumber']); ?></p>
                            <?php endif; ?>
                        </div>
                        <?php if ($card['isMain']): ?>
                            <span class="bg-yellow-500 text-xs px-2 py-1 rounded">Main</span>
                        <?php endif; ?>
                    </div>
                    <p class="text-2xl font-bold mb-4">$<?php echo number_format($card['currentBalance'], 2); ?></p>
                    <div class="flex space-x-2">
                        <a href="edit.php?id=<?php echo $card['idCard']; ?>" class="bg-blue-400 text-white px-3 py-1 rounded hover:bg-blue-300 transition">Edit</a>
                        <?php if (!$card['isMain']): ?>
                            <a href="delete.php?id=<?php echo $card['idCard']; ?>" onclick="return confirm('Delete this card?')" class="bg-red-400 text-white px-3 py-1 rounded hover:bg-red-300 transition">Delete</a>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endwhile; ?>
            <?php else: ?>
                <div class="col-span-3 bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-8 rounded text-center">
                    <p class="text-lg">No cards found. Add your first card to get started!</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
